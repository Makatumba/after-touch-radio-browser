...
export function renderStationCard(station: Station, state: State, t: Record<string, string>): string {
    ...
    return `<article class="station-card ${active ? 'active' : ''}"><div class="station-name">${station.name}</div><div class="station-badges"><span class="badge ${station.lastcheckok ? 'live' : ''}">${station.lastcheckok ? 'Reachable' : 'Unchecked / broken'}</span>${station.country ? `<span class="badge">${station.country}</span>` : ''}${station.language ? `<span class="badge">${station.language}</span>` : ''}</div><div class="station-actions">${playBtn}<button class="btn btn-secondary" data-fav="${station.stationuuid}">${isFavorite(station.stationuuid, state) ? t.remove : t.favorite}</button></div></article>`;
}
