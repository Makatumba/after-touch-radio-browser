...
export function renderStationCard(station: Station, state: State, t: Record<string, string>): string {
    ...
    const showPlay = !state.settings.disablePlayer && !state.settings.disablePlayButton;
    const playBtn = showPlay ? `<button class="btn btn-primary" data-play="${station.stationuuid}">${t.play}</button>` : '';
