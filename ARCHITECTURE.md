...
│       ├── remote.ts           # Live remote panel: now playing, transport (skip buttons toggleable), volume, mute, header ℹ device info + standby power button (replaced by reload/retry icon when configured speaker is reconnecting or unreachable — re-checks connection)
...
- **SoundTouch remote control**: with an address saved, the app keeps one WebSocket
  ...
  **standby power button** (wave 10): a fixed-size icon button in the header's upper-right
  ...
  signal), unaffected by `hideRemoteSkipButtons`, and labeled `remoteStandby` in all four
   languages. When the speaker is configured and either `soundtouchStatus === 'unreachable'` (offline banner) or `wsStatus === 'reconnecting'` (“Connection lost — retrying” before the 3-probe failure limit), the standby power button in the header is replaced by a reload button (↻, 24×24, `currentColor`, `aria-hidden="true"`, `focusable="false"`, fixed 48×48 — no layout shift) that is enabled even while disconnected; clicking it sets `soundtouchStatus` to `checking` (renders `⟳ Checking…`, reload icon stays disabled during the transient), re-checks reachability via `GET /info` (5s timeout, explicit port honored) and re-attempts the WebSocket (`ws://<host>:8080/`, “gabbo”) with stale-host guards (`state.soundtouchAddress === host`, `currentHost`), and ignores a second click while `checking`; on success the panel reverts to the power button (`wsStatus === 'connected'`), on failure it stays `unreachable` with the offline banner. When not configured or when reachable without `reconnecting`, behavior is unchanged and remaining controls stay gated by `wsStatus`. The retry label `remoteRetry` is localized in all four languages — no new keys. The now-playing parser
