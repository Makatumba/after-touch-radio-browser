...
│       ├── remote.ts           # Live remote panel: now playing, transport (skip buttons toggleable), volume, mute, header ℹ device info + standby power button (replaced by reload/retry icon when configured speaker is unreachable — re-checks connection)
...
- **SoundTouch remote control**: with an address saved, the app keeps one WebSocket
  ...
  **standby power button** (wave 10): a fixed-size icon button in the header's upper-right
  ...
  signal), unaffected by `hideRemoteSkipButtons`, and labeled `remoteStandby` in all four
   languages. When the speaker is configured but unreachable (`soundtouchStatus === 'unreachable'`), the standby power button in the header is replaced by a reload button (↻, 24×24, `currentColor`, `aria-hidden="true"`, `focusable="false"`, fixed 48×48 — no layout shift) that is enabled even while disconnected; clicking it sets `soundtouchStatus` to `checking` (renders `⟳ Checking…`), re-checks reachability via `GET /info` (5s timeout) and re-attempts the WebSocket connection with stale-host guards (`state.soundtouchAddress === host`, `currentHost`), and ignores a second click while `checking`; on success the panel reverts to the power button, on failure it stays `unreachable` with the offline banner. When not configured or when reachable, behavior is unchanged and next/prev/volume/mute/play-pause stay disabled as before. The retry label `remoteRetry` is localized in all four languages. The now-playing parser
