...
Controls are disabled while disconnected; the last-known state stays visible. Exception: when a speaker is configured and either `soundtouchStatus === 'unreachable'` (offline banner, FR-11) or `wsStatus === 'reconnecting'` (“Connection lost — retrying” before the banner — the gap covered by `PROBE_FAILURE_LIMIT=3`), the standby power button in the header is replaced by a reload ↻ (24×24, `currentColor`, `aria-hidden="true"`, `focusable="false"`, fixed 48×48 header button, no layout shift) that is enabled even while disconnected; clicking it sets `soundtouchStatus` to `checking` (renders `⟳ Checking…`), re-checks reachability via `GET /info` (5s timeout, explicit port honored) and re-attempts the WebSocket (`ws://<host>:8080/`, “gabbo”, `currentHost` stale-guard), and ignores a second click while `checking` (reload icon stays but disabled during the transient). On success the panel reverts to the power button, on failure it stays `unreachable` with the offline banner. The retry label `remoteRetry` is localized in all four languages. The backoff
